...
7. Launch with `scripts/research.py launch`. The harness exposes one physical GPU to each process, uses only GPUs 1 and 2, permits two jobs, and stops each job after 24 hours. After verifying supervisor identities, durable startup state, and the armed controller, use `$notify-wake` to enter an owned notify wait when the goal is active, the goal API permits blocking, and no immediate work remains.
8. Run `event-controller --study-id <study-id>` as the primary wake path for new launches. The v2 queue is under `logs/research/.notify-wake/v2/`; version-1 state is inert and unsupported. The controller waits on persisted notification retry deadlines without model polling, keeps fresh events deliverable while another event backs off, and records durable JSONL output under `logs/research/.event-controller/`. It may wake once after the first train-validation-checkpoint cycle, on a supervisor loss or progress stall, and on terminal state. Never keep a Codex turn open to sleep or poll. Keep the host and Codex app running.
