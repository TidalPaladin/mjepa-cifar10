...
7. Launch with `scripts/research.py launch`. The harness exposes one physical GPU to each process, uses only GPUs 1 and 2, permits two jobs, and stops each job after 24 hours. After verifying supervisor identities and durable startup state for the dispatched round, return the persistent goal to its event-wait state immediately when the goal API permits it.
8. Run `event-controller --study-id <study-id>` as the primary wake path for new launches. Direct Unix-socket delivery is the default, and the CLI discovers the running daemon socket. The controller waits on persisted notification retry deadlines without model polling, keeps fresh events deliverable while another event backs off, and records durable JSONL output under `logs/research/.event-controller/`. It may wake once after the first train-validation-checkpoint cycle, on a supervisor loss or progress stall, and on terminal state. Never keep a Codex turn open to sleep or poll. Keep the host and Codex app running.
9. Pin read-only scheduled fallback checks to GPT-5.6 Luna with medium reasoning instead of inheriting the chat default. The monitor may run `status` or `monitor --no-launch` and report failures even when `CIFAR10_DATA` is unavailable. The primary goal agent retains launches, summaries, promotion decisions, code and Git changes, and checkpoint deletion.
