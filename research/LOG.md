...
<!-- autoresearch-operation:{"content_sha256":"da8d6a1f61daa50cf58587c593686a7670c1be9bce339d9b3252062fc11744be","operation_id":"lejepa-token-diversity-v1-diagnostic-protocol-v1"} -->

## 2026-08-03 protocol: LeJEPA token-diversity bottleneck audit

- Studies: `lejepa-token-diversity-v1-audit` and `lejepa-token-diversity-v1-long-probe`; linked source: `lejepa-convergence-v2-long-horizon`.
- Question: Is the remaining teacher-free accuracy gap caused by within-image spatial token homogenization, probe calibration, or a representation deficit that appears earlier in the encoder?
- Hypothesis: The teacher-free encoder will retain sample-level variation but show excessive CLS-patch and patch-patch alignment, reduced centered patch energy, and weaker patch-route label utility in later transformer blocks. Fixed calibration will improve its `0.8150` online peak but remain at least `0.08` below the teacher's frozen accuracy.
- Sources: The retained seed-0 teacher baseline and retained seed-0 G2L2, SigREG lambda `0.05`, invariance weight `4.0`, 397-epoch shared-student checkpoint. No new pretraining occurs in this phase.
- Evaluation: Use only the fixed 45,000/5,000 split with deterministic validation transforms. The frozen probe reuses the established normalized final-two-CLS recipe and six fixed learning rates. The layerwise audit uses deterministic LayerNorm-cosine nearest-centroid probes on CLS, patch mean, and their concatenation, plus CLS-patch alignment, within-image patch cosine, centered patch energy and effective rank, and across-sample collapse statistics for every transformer block. The official test set is prohibited.
- Follow-up boundary: Use the audit to preregister no more than three mechanism-isolating candidates against one fresh early-horizon control. Early online AUC is a screening metric only; candidates must also improve the fixed frozen probe and spatial diagnostics before any longer evaluation.
- External tracking: W&B destination `tidalpaladin/mjepa-cifar10`, groups matching each study ID; each launch emits authorized non-sensitive `metrics`, `configs`, and `provenance` only.
- Resources and retention: At most two concurrent diagnostic workers on physical GPUs 1 and 2, two-hour diagnostic timeout, no official-test or supervised evaluation, and no destructive retention. Keep source checkpoints, feature caches, curves, layerwise results, and tracker records.
<!-- autoresearch-operation:{"content_sha256":"9cfec680ede4ab31e261709b3be46de931352d60b468fe31750bcff6da42da5c","operation_id":"lejepa-token-diversity-v1-long-probe-result-v1"} -->


## 2026-08-03 result: long-horizon probe calibration

- Study: `lejepa-token-diversity-v1-long-probe`; status: completed. Two GPU workers evaluated the teacher and strongest 400-epoch shared-student checkpoint under the same deterministic 100-epoch frozen-probe sweep. Both workers exited with code `0` after `78.86` wall seconds.
- Result: The teacher reached `0.9004` calibrated validation accuracy and the shared encoder reached `0.8270`, leaving a `0.0734` gap. Calibration changed the teacher by `-0.0054` and the shared encoder by `+0.0120` relative to their online probes. Probe under-training therefore accounts for only `0.0174` of the original `0.0908` online gap.
- Decision: The shared encoder exceeds the preregistered `0.80` representation floor, but its calibration gain is below the `0.05` probe-lag threshold and its teacher gap exceeds `0.05`. Classifier convergence is not the primary bottleneck. Continue with the preregistered layerwise representation and spatial-token audit before selecting objective ablations.
- Provenance: Parent `3c9eec032b9eae570fcd18c8a901f1c9bb46b5db`; manifest SHA-256 `7e220091854a1dad350ba284f54f77702f6493a83c2e22f667a749c8441fd329`; structured summary SHA-256 `49e0f4ba518c0499eebb186b5d55b5aa87c84be5c40289cd396a940ee8a57fb9`. Teacher W&B [run](https://wandb.ai/tidalpaladin/mjepa-cifar10/runs/wxmrfspt); shared W&B [run](https://wandb.ai/tidalpaladin/mjepa-cifar10/runs/pxiz2ybn).
- Recovery and retention: notify-wake watch `6274f7d3-21ff-4ba4-9341-05a1da5c2d46` validated PID `2429065` with start ticks `1252725761`, observed exit code `0`, and delivered the wake on its first attempt. Both source checkpoints, feature caches, probe curves, results, and tracker records remain retained. No destructive retention was applied.
<!-- autoresearch-operation:{"content_sha256":"b826a787e21622b120ecb76fa6aa6021669623a14721d7cc5b3e123e703a7f39","operation_id":"lejepa-token-diversity-v1-audit-result-v1"} -->


## 2026-08-03 result: LeJEPA layerwise token-diversity audit

- Study: `lejepa-token-diversity-v1-audit`; status: completed. The retained teacher and 397-epoch shared-student encoders were evaluated in `eval()` and `torch.inference_mode()` on the fixed 45,000/5,000 split. The official test set was not used.
- Final layer: Teacher versus shared CLS centroid accuracy was `0.8498` versus `0.7736`; patch-mean accuracy was `0.8512` versus `0.7438`. CLS-patch alignment was `0.48889` versus `0.92446`; within-image patch cosine was `0.45782` versus `0.99665`; centered patch energy was `0.53324` versus `0.00247`, a candidate-to-teacher ratio of `0.00463`.
- Localization: The shared encoder was competitive or better on patch-mean label utility through block 8. At block 9, within-image patch cosine rose to `0.93172`, centered-energy ratio fell to `0.0931` of the teacher, and the patch centroid route changed from an advantage at block 8 to a `0.0742` deficit. The deficit persisted through block 12 and reached `0.1074`.
- Decision: The preregistered spatial-homogenization and representation-gap gates both pass. The shared encoder retains sample-level semantic structure but blocks 9 through 12 nearly erase within-image patch residuals. Elevated CLS-patch alignment begins earlier, but it becomes a label-utility bottleneck only when late-block patch energy collapses.
- Follow-up: Screen one fresh early-horizon control against three mechanism-isolating candidates: CLS-only global SigREG/invariance, explicit full-view patch-residual SigREG, and reduced masked-prediction weight. Preserve shared-target gradients and the detached probe boundary in every run.
- Provenance: Parent `75eb82a3a3cf602a2cd3fa97758ccbb22bec7c42`; manifest SHA-256 `80e4062d4cea126f3d7c5df32a5f32f079575b1628a34402e3f5bf6360fd0224`; structured summary SHA-256 `a793bc8ed3b0cd9e73adcd034479929341c4ac3248babc5ecbe1de69387a00c5`. Teacher W&B [run](https://wandb.ai/tidalpaladin/mjepa-cifar10/runs/pbq50o3i); shared W&B [run](https://wandb.ai/tidalpaladin/mjepa-cifar10/runs/7pubvpl9).
- Recovery and retention: notify-wake watch `6937f6af-67d1-4918-b2be-d4d72672d249` validated PID `2432927` with start ticks `1252767304`, observed exit code `0`, released the owned wait, and delivered the wake on its first attempt. Both source checkpoints and all diagnostic artifacts remain retained.
<!-- autoresearch-operation:{"content_sha256":"ae175b062b5e90f9650d30c2a26ec9c0ff640ee4c1165030a8cef1a9403c8946","operation_id":"lejepa-token-diversity-v1-objective-screen-protocol-v1"} -->


## 2026-08-03 protocol: LeJEPA spatial-objective mechanism screen

- Studies: `lejepa-token-diversity-v1-objective-smoke` and `lejepa-token-diversity-v1-objective-screen`; linked audit: `lejepa-token-diversity-v1-audit`.
- Question: Does the teacher-free performance gap arise from patch-mean coupling in global invariance, missing within-image spatial regularization, or full-strength prediction against a gradient-carrying shared target?
- Control: Fresh seed-0 G2L2, lambda `0.05`, invariance weight `4.0`, CLS-plus-patch-mean direct SigREG, visual and auxiliary CLS masked prediction, AdamW learning rate `0.002`, weight decay `0.2`, 40 epochs, and validation every 5 epochs.
- Candidates: (1) change only global SigREG/invariance features to CLS; (2) add weight-`0.05`, 32-slice SigREG to centered patch tokens from every unmasked full view; (3) retain both masked prediction routes but reduce their common weight to `0.25`. Every target and view retains self-supervised gradients, while the online classifier input remains detached.
- Mechanical gate: One managed epoch combines reduced MIM weighting and patch-residual SigREG to validate finite reduction, gradients, raw and weighted logging, validation, checkpointing, W&B, and lifecycle notifications. Smoke accuracy is not scientific evidence.
- Scientific endpoints: Eight common-horizon validation records determine online intercept, slope, step AUC, active-time AUC, CLS-patch alignment, patch cosine, centered patch energy, centered patch effective rank, and across-sample collapse. All four terminal encoders receive the fixed normalized final-two-CLS 100-epoch six-rate probe bank before selection. The official test set is prohibited.
- Selection: A candidate must improve centered patch energy by at least `1.5x`, lower patch cosine by at least `0.05`, and either gain `0.02` frozen accuracy or remain within `0.005` while gaining `10%` step AUC versus the fresh control. Select at most one candidate, and only through a separately preregistered long-horizon study. Add no post-hoc route or weight variants.
- Resources and tracking: Four scientific pretraining runs at seed 0, at most two concurrent jobs on physical GPUs 1 and 2, 12-hour per-run timeout, W&B destination `tidalpaladin/mjepa-cifar10`, authorized metrics/configs/provenance only, and no destructive retention. Tandem `mjepa` commit `ef0ba60ab55eded71e665fb8cf2b1de0b493cce4` is local-only and must not be pushed without explicit permission.
<!-- autoresearch-operation:{"content_sha256":"aa90ba48a7426110697a2d8454125dd60555f8b4972b72197e84fdf98d8d8475","operation_id":"lejepa-token-diversity-v1-objective-logging-smoke-repair-v1"} -->


## 2026-08-03 mechanical repair: LeJEPA objective logging smoke

- Result: `lejepa-token-diversity-v1-objective-smoke` completed one epoch and optimizer step `44` with exit code `0`; validation, checkpointing, finite diagnostics, W&B finalization, and both lifecycle notifications succeeded. The terminal notification was accepted on its first delivery attempt.
...
<!-- autoresearch-operation:{"content_sha256":"6f761105ff8767329e967a54b2e824c28134b5293ba184cf59af13a2dfac6b5e","operation_id":"b68c7fa8503d0437978b747944ae8a55"} -->

<!-- study:lejepa-token-diversity-v1-objective-screen:phase:no-promotion -->
## lejepa-token-diversity-v1-objective-screen

- Question: Which late-block mechanism limits teacher-free masked LeJEPA convergence: pooled-feature coupling, missing spatial regularization, or shared-target MIM pressure?
- Hypothesis: Explicit patch-residual SigREG or reducing MIM weight will preserve the semantic gains through block 8 while preventing the block-9-to-12 spatial homogenization observed at long horizon; CLS-only global regularization will determine whether patch-mean invariance itself causes the coupling.
- Mechanisms and exact changes:
  - `g2l2-control`: Mechanism: Preserve the strongest teacher-free G2L2 objective and change only the horizon and validation cadence. Changes: Train for 40 epochs with validation every 5 epochs to resolve initial probe intercept and slope.
  - `g2l2-cls-only`: Mechanism: Remove only patch-mean features from global SigREG and invariance while retaining masked visual prediction and every shared-target gradient. Changes: Change sigreg_features from cls_patch_mean to cls.
  - `g2l2-patch-residual`: Mechanism: Apply a second SigREG term independently to centered patch tokens from every unmasked full view. Changes: Add patch-residual SigREG at weight 0.05 with 32 deterministic random slices.
  - `g2l2-mim025`: Mechanism: Retain visual and auxiliary CLS masked prediction but reduce only their common weight to 0.25; invariance and SigREG coefficients remain unchanged. Changes: Set jepa_loss_weight to 0.25 inside the LeJEPA prediction term.
- Launch code provenance:
  - `pretrain-g2l2-cls-only-seed0`: parent=`74bb194545d63151937b2f752f806d873f6637b0` (`codex/research/lejepa-token-diversity-v1`), mjepa=`ef0ba60ab55eded71e665fb8cf2b1de0b493cce4` (`codex/research/lejepa-token-diversity-v1`), vit=`bf15705454975f04912538cdc790d399eea69e67` (`codex/research/cls-context-routing-v1`)
  - `pretrain-g2l2-control-seed0`: parent=`74bb194545d63151937b2f752f806d873f6637b0` (`codex/research/lejepa-token-diversity-v1`), mjepa=`ef0ba60ab55eded71e665fb8cf2b1de0b493cce4` (`codex/research/lejepa-token-diversity-v1`), vit=`bf15705454975f04912538cdc790d399eea69e67` (`codex/research/cls-context-routing-v1`)
  - `pretrain-g2l2-mim025-seed0`: parent=`74bb194545d63151937b2f752f806d873f6637b0` (`codex/research/lejepa-token-diversity-v1`), mjepa=`ef0ba60ab55eded71e665fb8cf2b1de0b493cce4` (`codex/research/lejepa-token-diversity-v1`), vit=`bf15705454975f04912538cdc790d399eea69e67` (`codex/research/cls-context-routing-v1`)
  - `pretrain-g2l2-patch-residual-seed0`: parent=`74bb194545d63151937b2f752f806d873f6637b0` (`codex/research/lejepa-token-diversity-v1`), mjepa=`ef0ba60ab55eded71e665fb8cf2b1de0b493cce4` (`codex/research/lejepa-token-diversity-v1`), vit=`bf15705454975f04912538cdc790d399eea69e67` (`codex/research/cls-context-routing-v1`)
- Phase: no-promotion
- Winner: none
- External tracker: provider=W&B; account=tidalpaladin; project=mjepa-cifar10; authorized=True; approved_data_classes=metrics, configs, provenance
- Detail location: local summary and raw metrics under `/home/tidal/Documents/mjepa-cifar10/logs/research/lejepa-token-diversity-v1-objective-screen/summary.json`; external_detail=True
- Conclusion: No seed-0 candidate met a promotion threshold.
- Follow-up: record interpretation and the next falsifiable hypothesis.
- Checkpoint disposition: see each run below; deleted weights are not recoverable.

- `pretrain-g2l2-cls-only-seed0`: attempt=1; status=completed; decision=rejected; started=2026-08-03T01:08:21.892115+00:00; finished=2026-08-03T02:20:39.278740+00:00; terminal_event=6f3f9758-4fc5-4b3a-923b-d46aaf6029c6; artifacts=`/home/tidal/Documents/mjepa-cifar10/logs/research/lejepa-token-diversity-v1-objective-screen/runs/pretrain-g2l2-cls-only-seed0`; W&B=[run](https://wandb.ai/tidalpaladin/mjepa-cifar10/runs/56e66ea5); checkpoint=retained; metrics=peak_accuracy=0.417200, final_accuracy=0.417200, step_to_90=877, step_to_95=1097, active_seconds_to_90=2164.504, active_seconds_to_95=2701.981, step_auc=0.315505, active_time_auc=0.315218, active_seconds_at_step_horizon=4317.279, cls_path_latency_median_ms=30.977024, cls_path_latency_p90_ms=31.803392; error=none
- `pretrain-g2l2-control-seed0`: attempt=1; status=completed; decision=baseline; started=2026-08-03T01:08:21.806616+00:00; finished=2026-08-03T02:21:38.834510+00:00; terminal_event=30501b7a-3a8b-4ea6-aeab-15dd8b2ffbbb; artifacts=`/home/tidal/Documents/mjepa-cifar10/logs/research/lejepa-token-diversity-v1-objective-screen/runs/pretrain-g2l2-control-seed0`; W&B=[run](https://wandb.ai/tidalpaladin/mjepa-cifar10/runs/0d881cac); checkpoint=retained; metrics=peak_accuracy=0.344600, final_accuracy=0.344600, step_to_90=1535, step_to_95=1535, active_seconds_to_90=3832.486, active_seconds_to_95=3832.486, step_auc=0.284523, active_time_auc=0.283544, active_seconds_at_step_horizon=4377.843, cls_path_latency_median_ms=31.488000, cls_path_latency_p90_ms=32.234497; error=none
- `pretrain-g2l2-mim025-seed0`: attempt=1; status=completed; decision=rejected; started=2026-08-03T02:22:11.988112+00:00; finished=2026-08-03T03:35:54.312093+00:00; terminal_event=578cef39-5fb3-4737-9d4c-820fc1affae0; artifacts=`/home/tidal/Documents/mjepa-cifar10/logs/research/lejepa-token-diversity-v1-objective-screen/runs/pretrain-g2l2-mim025-seed0`; W&B=[run](https://wandb.ai/tidalpaladin/mjepa-cifar10/runs/116ab17e); checkpoint=retained; metrics=peak_accuracy=0.361200, final_accuracy=0.361200, step_to_90=1097, step_to_95=1316, active_seconds_to_90=2738.051, active_seconds_to_95=3284.225, step_auc=0.300746, active_time_auc=0.299806, active_seconds_at_step_horizon=4375.365, cls_path_latency_median_ms=31.723520, cls_path_latency_p90_ms=32.652287; error=none
- `pretrain-g2l2-patch-residual-seed0`: attempt=1; status=completed; decision=rejected; started=2026-08-03T02:21:25.418016+00:00; finished=2026-08-03T03:36:18.652490+00:00; terminal_event=900d98d3-64f1-4f82-9637-de19802c5741; artifacts=`/home/tidal/Documents/mjepa-cifar10/logs/research/lejepa-token-diversity-v1-objective-screen/runs/pretrain-g2l2-patch-residual-seed0`; W&B=[run](https://wandb.ai/tidalpaladin/mjepa-cifar10/runs/a8ecaf51); checkpoint=retained; metrics=peak_accuracy=0.389800, final_accuracy=0.389800, step_to_90=1097, step_to_95=1097, active_seconds_to_90=2780.850, active_seconds_to_95=2780.850, step_auc=0.310950, active_time_auc=0.308583, active_seconds_at_step_horizon=4441.531, cls_path_latency_median_ms=31.222272, cls_path_latency_p90_ms=31.867905; error=none
<!-- autoresearch-operation:{"content_sha256":"addc44b9736a4cbcf77a988208fdd8f8d57ee8e9936d376bcd5475bbcdf1b2ca","operation_id":"lejepa-token-diversity-v1-objective-screen-result-v1"} -->


## 2026-08-03 result: LeJEPA spatial-objective mechanism screen

- Scope: `lejepa-token-diversity-v1-objective-screen` completed the fresh control and all three seed-0 candidates for 40 epochs. `lejepa-token-diversity-v1-objective-probe` then evaluated every frozen encoder with the fixed normalized final-two-CLS, six-rate, 100-epoch probe bank. All runs used the fixed 45,000/5,000 split; the official test set was not used.
- Control: Online final accuracy was `0.3446`, frozen accuracy was `0.4630`, within-image patch cosine was `0.996044`, and centered patch energy was `0.00324406`. The last-three minimum CLS and patch-mean effective-rank fractions were `0.03017` and `0.02881`, below the committed `0.07` floors.
- CLS-only: Removing patch means from global SigREG and invariance raised online accuracy to `0.4172`, frozen accuracy to `0.5544` (`+0.0914`), and structured step AUC by `12.97%`. Spatial collapse became worse: patch cosine rose to `0.998332`, centered energy fell to `0.00172110` (`0.531x` control), patch-mean std fell below `0.003`, and patch-mean sample cosine exceeded `0.9993`. This candidate failed the collapse, energy, and cosine gates.
- Reduced MIM: Weighting masked visual and auxiliary prediction by `0.25` produced online accuracy `0.3612`, frozen accuracy `0.4898` (`+0.0268`), and a `5.69%` step-AUC gain. Centered energy rose `2.269x`, but patch cosine improved by only `0.00574`, below the required `0.05`; both effective-rank floors also failed.
- Patch-residual SigREG: Adding the weight-`0.05`, 32-slice centered-patch term produced online accuracy `0.3898`, frozen accuracy `0.4988` (`+0.0358`), and a `9.27%` step-AUC gain. Patch cosine fell to `0.389919`, an improvement of `0.606124`, while centered energy rose to `0.587535` (`181.11x` control). It passed both spatial gates and the frozen-accuracy gate. It did not qualify because the last-three minimum CLS and patch-mean effective-rank fractions were `0.03750` and `0.02156`, below `0.07`.
- Diagnosis: At seed 0, missing within-image spatial regularization caused part of the accuracy gap. Shared-target masked-prediction pressure also contributed, but reducing it did not stop homogenization. Patch-mean global regularization was not the cause: removing it improved CLS label utility while making patch collapse more severe. The remaining measured bottleneck is low across-sample effective rank, which patch-residual SigREG does not address.
- Decision: No candidate passed every preregistered gate, so `selected_source=null` and no long-horizon run is authorized from this screen. A future study can test patch-residual SigREG together with an independent across-sample rank-preserving intervention, but that requires a new protocol.
...
<!-- autoresearch-operation:{"operation_id":"lejepa-patch-rank-v1-screen-result-v1"} -->

## 2026-08-03 result: cross-sample patch-rank screen

- Scope: The fresh patch-residual control and all three preregistered seed-0 candidates completed 40 epochs and 1,755 optimizer steps with finite metrics. The fixed normalized final-two-CLS, six-rate, 100-epoch frozen probe and deterministic layerwise centroid diagnostics then evaluated all four encoders on the fixed 45,000/5,000 split. The official test set was not used.

| Variant | Online final | Step AUC | Last-three patch-rank ratio | Final patch-centroid gain | Energy ratio | Patch-cosine increase | Frozen accuracy | Frozen loss | Failed gates |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| patch-residual control | `0.3898` | `0.31841` | `1.000x` | reference | `1.000x` | reference | `0.4988` | reference | none |
| global SigREG lambda `0.10` | `0.3486` | `0.30692` | `1.816x` | `-0.0386` | `0.963x` | `+0.02496` | `0.4578` | `0.0410` | patch centroid, frozen accuracy, step AUC |
| patch-sample weight `0.01` | `0.3584` | `0.30348` | `1.328x` | `-0.0496` | `0.898x` | `+0.06084` | `0.4540` | `0.0448` | patch rank, patch centroid, patch cosine, frozen accuracy, step AUC |
| patch-sample weight `0.05` | `0.3548` | `0.31741` | `1.520x` | `-0.0508` | `0.831x` | `+0.11587` | `0.4536` | `0.0452` | patch centroid, patch cosine, frozen accuracy |

- Rank response: Doubling global SigREG and the stronger patch-sample term passed the `1.50x` last-three patch-mean rank gate. The weaker patch-sample term reached only `1.328x`. All four encoders passed every finite-output and non-collapse safety threshold.
- Label-utility response: Rank did not translate into semantic patch information. Final patch-mean centroid accuracy was `0.3124` for the control, `0.2738` for global lambda `0.10`, `0.2628` for patch-sample `0.01`, and `0.2616` for patch-sample `0.05`. Patch accuracy decreased from block 8 to block 12 for every encoder; the changes were `-0.0070`, `-0.0090`, `-0.0104`, and `-0.0014` in the same order.
- Representation response: The control reached `0.4988` frozen accuracy. Candidate accuracies of `0.4578`, `0.4540`, and `0.4536` lost `0.0410` to `0.0452`, far beyond the committed `0.005` budget. The stronger cross-sample term retained only `83.1%` of control centered patch energy and increased within-image patch cosine by `0.11587`, so isotropic rank pressure also weakened the spatial information restored by patch-residual SigREG.
- Interpretation: Global or position-wise cross-sample SigREG can distribute patch means across more directions without aligning those directions with class structure. The measured rank deficit was a correlate, not the causal semantic bottleneck. More isotropic pressure trades away frozen CLS quality and, for the patch-sample variants, part of the within-image spatial recovery. A future mechanism study should target local semantic correspondence or content-aware patch routing rather than increasing rank alone.
- Decision: `selected_source=null`; no candidate passed every committed gate. Stop without a longer-horizon run, confirmation, supervised evaluation, or official-test evaluation. Do not add a post-hoc lambda, slice-count, crop, or patch-sample variant to this screen.
- Provenance: Training parent `e1647c8d53fa965ef3217c5580fa3df11a2d9805`; evaluation parent `a6dcbe09c726d112902ef7220655d60c796e6798`; `mjepa=52f898bcc9f44d2f59199da359fa74438f2724ed`; `vit=bf15705454975f04912538cdc790d399eea69e67`. Screen manifest SHA-256 `4e950b3941a093a260df1a84d33abf65a5ae556d87c0530dd1e8b4f91b163bf5`; managed summary `6334602d576fbebbb2d84b5ed01e725f85a91b968b4474865d37adef0e490661`; probe manifest `89747f7a482b6cd4cc77e09b4207334b58e7bf2f9acd8df994f3f4a15974efa6`; diagnostic manifest `2fe605a7733dda2856754010778c19c20a07b508c82e07b2ec2160817a1fe3c0`; structured decision `d38288e5a5394bd9c8a00a2ea4ee877f0826bafd8d7f011bc9c48ebc13e0723b`.
- Tracking and cost: Pretraining consumed `17,934.73` summed active seconds across four runs over `2h 32m 14s`. Probe training consumed `261.36` summed GPU seconds in `141.50` wall seconds; diagnostics consumed `64.92` summed GPU seconds. Total measured GPU-active time was `5.073` hours. Pretraining W&B runs were `3514570d`, `dba2dc8a`, `ed762343`, and `be04a754`; probe runs were `xoypuit4`, `fisz3ff0`, `jbzvnuds`, and `j5xoseps`; diagnostic runs were `dy5oj7xr`, `i2kd4loh`, `6t3w3kio`, and `jyzhn4l3`.
- Recovery and retention: The study-scoped controller delivered every first-cycle and terminal pretraining event. notify-wake watches `b45b9d5f-8d58-4fac-895c-0718c0061283` and `e78f29ac-d796-44ee-aaaf-c1f3aea7357c` validated the exact probe and diagnostic processes, observed successful exit, and released their owned goal waits. Every checkpoint, backbone, feature cache, curve, result, summary, tracker record, and notification artifact remains retained; no destructive retention was applied.

<!-- autoresearch-operation:{"operation_id":"lejepa-target-stopgrad-v1-protocol-v1"} -->

## 2026-08-03 protocol: shared target stop-gradient without EMA

- Question: Does preventing the shared full-view target from following masked-prediction error recover label-bearing patch and global information without an EMA parameter copy?
- Fixed reference: Reuse the exact retained seed-0 `pretrain-patch-residual-control-seed0` curve and checkpoint from `lejepa-patch-rank-v1-screen`. The committed eight-point metric curve has SHA-256 `c6608eacb35312fe331698cde4714e73c241996fc0d415c523c6ccfd2f4cff0d`.
- Candidate: One seed-0, 40-epoch G2L2 run changes only `stop_gradient_target=true`. The full-view target is produced by the shared student in train mode. Visual targets are detached only at visual and auxiliary-CLS masked-prediction losses; target/additional-view SigREG, multiview invariance, and patch-residual SigREG retain gradients. No teacher module or EMA state is created. The online probe boundary remains detached.
- Evaluation: Compare first, peak, final, and step-AUC online accuracy at the common horizon; then apply the fixed normalized final-two-CLS six-rate frozen probe and deterministic layerwise CLS, patch-mean, and combined centroid diagnostics. The official test set is prohibited.
...
<!-- autoresearch-operation:{"content_sha256":"7a5aa8e0173a5447b89a62a308d45619bc65e157f0f27f1d0a4ad224796d88f3","operation_id":"lejepa-single-view-stopgrad-v1-protocol-v1"} -->
## 2026-08-03 protocol: crop-free single-view stop-gradient MIM

- Question: Can the stopped shared-target mechanism preserve useful global and patch representations using one image only, with no global/local crop pair and no multiview invariance loss?
- Scientific pair: Launch fresh seed-0, 40-epoch shared-gradient and stopped-target runs concurrently. Both use deterministic resize instead of `RandomResizedCrop`, one global input, zero local inputs, one full and one masked forward of that same augmented image, effective batch size 1,024, the existing visual and auxiliary-CLS MIM losses, direct CLS-plus-patch-mean SigREG, and patch-residual SigREG. The pair differs only in `stop_gradient_target`.
- Evaluation: Compare online first/final/peak/step-AUC, the fixed normalized final-two-CLS frozen probe, and all-layer CLS/patch/combined centroid diagnostics. The immutable multiview stopped-target reference is commit `db26c134dadda811396fe9db554a5a6572b1510f`: online final `0.4804`, step AUC `0.396798`, frozen accuracy `0.6034`, final patch centroid `0.3918`, centered patch energy `0.553215`, and patch cosine `0.427148`.
...
<!-- autoresearch-operation:{"content_sha256":"d04593b04cb390f54230fd926b1e074b0b000af7e8e35bcb12e0b641e187870d","operation_id":"lejepa-single-view-stopgrad-v1-gate-v1"} -->

## 2026-08-03 result: crop-free single-view stop-gradient gate

- Safety: Passed. Both fresh 40-epoch runs were finite and the stopped-target run satisfied every preregistered collapse threshold.
- Fresh-control response: Stop-gradient raised terminal online accuracy from `0.3174` to `0.3330` (`+0.0156`) and fixed frozen accuracy from `0.4088` to `0.4278` (`+0.0190`). It lowered first-validation accuracy from `0.2228` to `0.1846` (`-0.0382`), left step AUC effectively unchanged (`0.264529` versus `0.264549`), and raised final patch-mean centroid accuracy from `0.3070` to `0.3298` (`+0.0228`). Centered patch energy improved by `1.1249x` and within-image patch cosine fell by `0.0411`.
- Mechanism gate: Failed. The frozen-accuracy and spatial-retention thresholds passed, but first-validation gain missed the required `+0.01` and final patch-centroid gain missed the required `+0.03`. The evidence supports a useful late-horizon effect, not the complete preregistered target-chasing mechanism claim.
- Multiview comparison: Crop-free equivalence failed. Relative to the retained multiview stopped-target reference, the crop-free candidate lost `0.1474` final online accuracy, `0.132269` step AUC, `0.1756` frozen accuracy, and `0.0620` final patch-centroid accuracy. Its centered patch-energy ratio was `0.8750`, which passed, but patch cosine increased by `0.0815`, above the allowed `0.05`.
- Decision: Continue only with the two preregistered seed-0 bottleneck ablations. The global deficit independently warrants `lejepa_lambda=0.10`; the patch-centroid and patch-cosine deficits independently warrant `patch_residual_sigreg_loss_weight=0.10`. Each candidate changes exactly one objective weight from the stopped-target control. No confirmation, fine-tuning, or official-test evaluation is authorized.
- Provenance: Screen manifest `b9c7bfc198b7de4dca410a1c9cc5c68d1aca2ea47591a5b2d43d821c31723c05`; managed summary `02a0ab933150f94137a62c7608b65c22be26f37fddc07d78f110edd8e23c4487`; frozen-probe manifest `f70c877077bf2434dfca12b3b4c2f2d6c616dfa9b622d729c69ae0e2d8956e3e`; frozen-probe summary `6fba97b4bb84c73a8c8dde4b0987a407d8ffe00ccfb8d9d23f341e4ac210013d`; diagnostic manifest `95ea8205b8d321a79f69b14fd9bd963287f4470e671f57486dcaf2166b66632e`; diagnostic summary `9fcecade4f805c45a49271fd25d653f3b482cc02450235b2b1580f47628650a1`. W&B pretraining runs were `f0703395` and `083f32c6`; probe runs were `tse31f6u` and `bglogsv6`; diagnostic runs were `u4xg5q34` and `6ux8uu4b`. Every checkpoint, backbone, feature cache, metric curve, W&B record, lifecycle event, and notification artifact remains retained.
<!-- autoresearch-operation:{"content_sha256":"89d6013673654081f6133ba1b51e227b93f77ab7bebefd4f8675e749d1dc2f63","operation_id":"lejepa-single-view-stopgrad-v1-bottleneck-protocol-v1"} -->

## 2026-08-03 protocol: crop-free bottleneck screen

- Trigger: The safe crop-free stopped-target run improved fresh-control final online and frozen accuracy but failed crop-free equivalence. Both preregistered deficits were independently present: frozen accuracy was `0.1756` below the multiview reference, final patch-centroid accuracy was `0.0620` below it, and patch cosine exceeded it by `0.0815`.
- Candidate pair: Launch exactly two seed-0, 40-epoch runs concurrently. `single-view-stopgrad-lambda010` changes only `lejepa_lambda` from `0.05` to `0.10`. `single-view-stopgrad-patch-residual010` changes only `patch_residual_sigreg_loss_weight` from `0.05` to `0.10`. The immutable crop-free stopped-target control curve is `research/baselines/lejepa-single-view-stopgrad-v1-control-seed0.metrics.jsonl` with SHA-256 `68def35771f8f583b4a2964a55633c042a0c268cd3ebc71a711167da4bc99254`.
- Evaluation: Compare the full online curve, fixed normalized final-two-CLS frozen probe, and all-layer CLS, patch-mean, and combined centroid diagnostics. The global-path response requires at least `+0.01` frozen accuracy with no more than `0.005` online loss and `0.01` patch-centroid loss. The patch-path response requires at least `+0.01` patch-centroid accuracy with no more than `0.005` frozen or online loss. Both retain the existing collapse, patch-energy, and patch-cosine limits. Crop-free equivalence retains every original `0.03` tolerance versus the immutable multiview reference.
- Stop rule: These are the last two authorized pretraining runs in this program. Do not combine weights, confirm, fine-tune, or evaluate the official test set. Retain every artifact.
- Preregistration: Study `b0b97ffcf0b9389dfe4510b7a7789c635661c804f3db0f637313ae978815dfc1`; frozen-probe manifest `eb131d212abaa617dcf957cff21630405f234078765ca5af03841016b21370ba`; diagnostic manifest `a99ffba38eea5c3c4f4f7c08ca227c3975e248b5fe0c67f2228038633ca23034`. Track authorized metrics, configs, and provenance in `tidalpaladin/mjepa-cifar10`.
<!-- autoresearch-operation:{"content_sha256":"3fbab878fc1ef84eb912073b66ec772df1b2d37c92af71ad914f8fa09cd5fe23","operation_id":"lejepa-single-view-stopgrad-v1-bottleneck-probe-prose-amendment-v1"} -->
...
<!-- autoresearch-operation:{"content_sha256":"2e5943997f628e079ed7d17fd27170a3364e42010cbe440716cc95e9426fafbd","operation_id":"lejepa-single-view-stopgrad-v1-final-v1"} -->
## 2026-08-03 final result: crop-free single-view stopped-target MIM

- Outcome: A shared-weight stopped target trains without collapse using one un-cropped image and no EMA teacher, but it does not match the retained multiview stopped-target reference. Both preregistered SigREG-strength follow-ups failed their pathway-response and crop-free-equivalence gates. No candidate is promoted.
- Exact design: The crop-free pair uses one 32x32 image per example, deterministic resize, non-spatial stochastic augmentation, one full-view and one masked-context forward, the visual-token and partitioned auxiliary-CLS masked-prediction losses, direct CLS-plus-patch-mean SigREG, patch-residual SigREG, no multiview-invariance loss, and no teacher parameters or EMA update. The online classifier consumes an explicitly detached final full-view CLS token.

| Run | Role | Online final | Step AUC | Frozen probe | Final patch centroid | Patch energy | Patch cosine |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Shared-gradient control | crop-free control | `0.3174` | `0.264549` | `0.4088` | `0.3070` | `0.430321` | `0.549663` |
| Stopped target | crop-free control candidate | `0.3330` | `0.264529` | `0.4278` | `0.3298` | `0.484056` | `0.508608` |
| Multiview stopped target | immutable reference | `0.4804` | `0.396798` | `0.6034` | `0.3918` | `0.553215` | `0.427148` |
| Direct SigREG `lambda=0.10` | bottleneck candidate | `0.2970` | `0.255639` | `0.4142` | `0.3030` | `0.489843` | `0.503506` |
| Patch-residual SigREG `weight=0.10` | bottleneck candidate | `0.3112` | `0.269486` | `0.4216` | `0.3144` | `0.510544` | `0.479296` |

- Non-collapse: Every crop-free run passed the finite-fraction, CLS variance/eigenspectrum/cosine, patch-mean variance/eigenspectrum/cosine, and visual-target-relative-improvement limits. At the final validation, the lambda candidate had target-CLS standard deviation `0.7821`, top-eigenvalue fraction `0.0416`, and cosine `0.00836`; the patch-residual candidate had `0.8095`, `0.0352`, and `0.01143`. Both had visual-target relative improvement above `0.61`.
- Shared-control response: Stop-gradient improved final online accuracy by `+0.0156`, frozen accuracy by `+0.0190`, and final patch-centroid accuracy by `+0.0228`; centered patch energy rose `1.1249x` and patch cosine fell `0.0411`. The full mechanism gate still failed because first-validation accuracy fell by `0.0382` instead of improving by `0.01`, and patch-centroid gain missed the required `+0.03` by `0.0072`. Step AUC was unchanged within `0.000021`.
- Crop-free equivalence: The stopped-target crop-free run lost `0.1474` online-final accuracy, `0.132269` step AUC, `0.1756` frozen accuracy, and `0.0620` final patch-centroid accuracy versus multiview. Its patch-energy ratio was `0.874986x`, which passed the `0.80x` floor, but patch cosine increased `0.08146`, exceeding the `0.05` limit. Overall equivalence failed.
- Direct-SigREG response: `lambda=0.10` was safe but reduced frozen accuracy by `0.0136`, online final by `0.0360`, step AUC by `0.008890`, and patch-centroid accuracy by `0.0268` versus the crop-free control. Its pathway gate and five of six multiview-equivalence endpoints failed.
- Patch-residual response: `weight=0.10` was safe and improved step AUC by `0.004958`, but reduced patch-centroid accuracy by `0.0154` instead of gaining `0.01`, reduced frozen accuracy by `0.0062`, and reduced online final by `0.0218`. Its pathway gate and five of six multiview-equivalence endpoints failed.
- Layerwise diagnosis: At block 8, the lambda and patch-residual candidates were respectively `0.0064` and `0.0218` below the crop-free control in patch-centroid accuracy. At the final block they were `0.0268` and `0.0154` below. Stronger patch residual reduced final within-image cosine by `0.0293` and increased centered energy `1.0547x`, but that geometric improvement did not become label-aligned patch information.
- Interpretation: Stopping target gradients removes part of the target-chasing shortcut and is a viable collapse-prevention design, but view diversity remains the supported performance bottleneck. Doubling direct or patch-residual SigREG changes representation geometry without restoring semantic intercept, frozen CLS quality, or patch-label utility. The evidence does not prove that literal crops are theoretically necessary; it shows that the tested single-view formulation needs multiview crops or another source of independent view diversity to match the retained reference at 40 epochs.
- Decision and scope: `crop-free-single-view-stopgrad-is-safe-but-not-equivalent`. Stop this program with no confirmation, combined-weight candidate, fine-tuning, or official-test evaluation. All scientific results are seed-0 screens and are not confirmation estimates.
- Structured result: `research/diagnostics/lejepa-single-view-stopgrad-v1-result-v1.json`. Screen summary SHA-256 `02a0ab933150f94137a62c7608b65c22be26f37fddc07d78f110edd8e23c4487`; screen probe summary `6fba97b4bb84c73a8c8dde4b0987a407d8ffe00ccfb8d9d23f341e4ac210013d`; screen diagnostic summary `9fcecade4f805c45a49271fd25d653f3b482cc02450235b2b1580f47628650a1`; bottleneck summary `1e8242e30990c2db535b7519384506d7daab90509263447247d939dc1ccb2a8c`; bottleneck probe summary `c77ac1be82424aadcd9c4419eacbb1b823a19bd14a72ba5eabae9f4e104928d1`; bottleneck diagnostic summary `0c57a75f319165e1924e3faed6297a620b3cfe6dd977312ad5dfc02bee722b79`.
- Tracking: Crop-free pretraining runs `f0703395`, `083f32c6`, `e3970238`, and `c5e2a0dc`; frozen-probe runs `tse31f6u`, `bglogsv6`, `kd8e4wua`, `ehjtw4al`, and `v4g70d1r`; diagnostic runs `u4xg5q34`, `6ux8uu4b`, `ixioa26g`, `i6yg5mff`, and `skj4u4q4`. All used online W&B in `tidalpaladin/mjepa-cifar10` for the authorized metrics, configs, and provenance classes.
...
<!-- autoresearch-operation:{"content_sha256":"3611016e1c1edce457a2c45600e3e020b396cbdfb0d72e0213cd0958c67f109c","operation_id":"12f4e60b799a4a8a9e5aa2dbee1b6025"} -->

<!-- study:ijepa-token-specialization-v1-screen:phase:no-promotion -->
## ijepa-token-specialization-v1-screen

- Question: Does separating CLS/register and visual-token parameter paths reduce global homogenization of I-JEPA patch tokens while preserving global representation quality?
- Hypothesis: Separate pre-attention and pre-MLP normalization plus LayerScale in all blocks, together with separate QKV projections in the first third of the encoder, will lower CLS-patch alignment and within-image patch similarity, improve patch-mean label utility, and preserve the baseline CLS probe curve.
- Mechanisms and exact changes:
  - `ijepa-control`: Mechanism: Train the existing EMA-teacher masked-prediction objective with one CLS token, seven register tokens, and fully shared encoder blocks. Changes: not recorded.
  - `token-specialized`: Mechanism: Treat CLS and register tokens as one global stream; split its normalization, LayerScale, and early QKV parameters from visual-token parameters while retaining shared token interaction through attention. Changes: Use separate pre-attention and pre-MLP LayerNorm parameters in all 12 encoder blocks.; Use separate attention and MLP LayerScale parameters in all 12 encoder blocks.; Use separate QKV projections in blocks 1 through 4, the first third of the encoder.; Clone every visual branch from the corresponding global branch so both architectures are functionally identical at initialization under a paired seed.; Keep attention, output projections, MLP projections, final output norm, predictor, EMA teacher, masks, augmentations, optimizer, and loss weights unchanged.
- Launch code provenance:
  - `pretrain-ijepa-control-seed0`: parent=`23a9ec3f103dfb4d6c6b9adb6254421b4aa025de` (`codex/research/ijepa-token-specialization-v1`), mjepa=`578a2f92441394f232732348b6e9f569237b2744` (`codex/research/lejepa-target-stopgrad-v1`), vit=`f3b114c391414b2c2a4a0f4e04d7e9cadf9301ec` (`codex/research/ijepa-token-specialization-v1`)
  - `pretrain-token-specialized-seed0`: parent=`77583e6946d60d4cc801ae543002b3382266c120` (`codex/research/ijepa-token-specialization-v1`), mjepa=`578a2f92441394f232732348b6e9f569237b2744` (`codex/research/lejepa-target-stopgrad-v1`), vit=`6d298af42b88f5d734554df28fef0198d6b0aed3` (`codex/research/ijepa-token-specialization-v1`)
- Phase: no-promotion
- Winner: none
- External tracker: provider=W&B; account=tidalpaladin; project=mjepa-cifar10; authorized=True; approved_data_classes=metrics, configs, provenance
- Detail location: local summary and raw metrics under `/home/tidal/Documents/mjepa-cifar10/logs/research/ijepa-token-specialization-v1-screen/summary.json`; external_detail=True
- Conclusion: No seed-0 candidate met a promotion threshold.
- Follow-up: record interpretation and the next falsifiable hypothesis.
- Checkpoint disposition: see each run below; deleted weights are not recoverable.

- `pretrain-ijepa-control-seed0`: attempt=1; status=completed; decision=baseline; started=2026-08-04T00:38:24.894985+00:00; finished=2026-08-04T03:45:01.578693+00:00; terminal_event=62cd6610-391f-430e-aa15-70959d3a0937; artifacts=`/home/tidal/Documents/mjepa-cifar10/logs/research/ijepa-token-specialization-v1-screen/runs/pretrain-ijepa-control-seed0`; W&B=[run](https://wandb.ai/tidalpaladin/mjepa-cifar10/runs/3d9e5f35); checkpoint=retained; metrics=peak_accuracy=0.898800, final_accuracy=0.898200, step_to_90=6090, step_to_95=7830, active_seconds_to_90=3912.176, active_seconds_to_95=5029.942, step_auc=0.794415, active_time_auc=0.794253, active_seconds_at_step_horizon=11176.521, cls_path_latency_median_ms=31.441920, cls_path_latency_p90_ms=32.397312; error=none
- `pretrain-token-specialized-seed0`: attempt=3; status=completed; decision=rejected; started=2026-08-04T01:31:33.861998+00:00; finished=2026-08-04T05:00:04.090785+00:00; terminal_event=1d496d30-be0b-4f7b-bfd2-35758f48236e; artifacts=`/home/tidal/Documents/mjepa-cifar10/logs/research/ijepa-token-specialization-v1-screen/runs/pretrain-token-specialized-seed0`; W&B=[run](https://wandb.ai/tidalpaladin/mjepa-cifar10/runs/41b50b9d); checkpoint=retained; metrics=peak_accuracy=0.902000, final_accuracy=0.901200, step_to_90=6525, step_to_95=9570, active_seconds_to_90=4703.034, active_seconds_to_95=6893.080, step_auc=0.786950, active_time_auc=0.772843, active_seconds_at_step_horizon=12495.243, cls_path_latency_median_ms=30.872065, cls_path_latency_p90_ms=31.818752; error=none
<!-- autoresearch-operation:{"content_sha256":"344d1a26f3b7e1edd80c0ed299abf885d2bedf3e1e6985cf86e64e24d738285d","operation_id":"ijepa-token-specialization-v1-result-v1"} -->
## I-JEPA token specialization v1 final result (2026-08-04)

- Outcome: `mechanism-changed-but-support-gate-not-passed`. Separate global-token and visual-token normalization, LayerScale, and early QKV paths produced the intended token separation and preserved global quality, but the final patch-mean nearest-centroid gain was +0.0062 against the preregistered +0.0100 minimum.
- Online convergence at the common 17,400-step horizon: control first/peak/final accuracy was 0.3714/0.8988/0.8982; specialization was 0.3720/0.9020/0.9012. Specialization gained 0.0032 peak and 0.0030 final accuracy, while step AUC fell from 0.7944 to 0.7870, step-to-95 increased from 7,830 to 9,570, and active time increased 11.80%.
- Final frozen diagnostics: CLS centroid accuracy increased from 0.8520 to 0.8610 and patch-mean centroid accuracy increased from 0.8414 to 0.8476. CLS-patch alignment fell from 0.5623 to -0.0025, within-image patch cosine fell from 0.4436 to 0.3442, and centered patch energy increased from 0.5395 to 0.6435 (ratio 1.1927). Both encoders passed every final safety threshold.
- Preregistered gate: online peak loss, final CLS-centroid loss, CLS-patch alignment decrease, patch-cosine decrease, centered-patch energy, and both safety gates passed. Only `final-patch-mean-centroid-accuracy-gain` failed: +0.0062 observed versus +0.0100 required.
- Layerwise interpretation: specialization changes routing from block 1, but dense utility is not uniformly better. Patch-mean accuracy is below control in blocks 2, 6-9, reaches its largest deficit at block 8 (-0.0296), and recovers to a small gain in blocks 10-12. The largest gain occurs at block 5 (+0.0142). This supports a real geometric mechanism change without evidence for a material dense-semantic improvement.
- Mechanical retries: candidate attempts 1 and 2 stopped before scientific training because of a compiled symbolic-backward failure and an immutable W&B launch-benchmark retry failure. Attempt 3 completed after the recorded mechanical repairs; the objective and endpoints were unchanged.
- Provenance: control parent `23a9ec3f103dfb4d6c6b9adb6254421b4aa025de`, control vit `f3b114c391414b2c2a4a0f4e04d7e9cadf9301ec`; specialized and diagnostic parent `77583e6946d60d4cc801ae543002b3382266c120`, specialized vit `6d298af42b88f5d734554df28fef0198d6b0aed3`; mjepa `578a2f92441394f232732348b6e9f569237b2744`. Fixed split SHA-256 `bf789cefb53952724b4c8674eb1a7dbcd164a0b5ec843a30e466ce6e1239b86b`.
- Compute and artifacts: 6.5981 total GPU-active hours including smoke and diagnostics. Both checkpoints, both complete curves, all layerwise diagnostics, W&B records, and lifecycle evidence are retained. Structured result: `research/diagnostics/ijepa-token-specialization-v1-result-v1.json`.
- Scope and stop: this is a paired seed-0 mechanism screen, not a confirmation estimate. Stop without confirmation, fine-tuning, or official CIFAR-10 test evaluation.

### Post-hoc patch-token PCA and spatial-coherence diagnostic

- Scope: Exploratory final-layer analysis of the two retained seed-0 checkpoints on the fixed 5,000-image validation holdout. This analysis does not change the completed promotion decision and does not use the official test set.
- PCA method: Fit an independent three-component PCA basis for each model to within-image-centered visual tokens from 640 stratified validation images, then display one disjoint validation example per class. Because the bases are independent, spatial regions and boundaries can be compared, but literal colors cannot be matched between models.
- Raw spatial contrast: Baseline adjacent/nonadjacent cosine was `0.7582/0.4251`, for neighbor excess `0.3331`. Specialization was `0.7201/0.3221`, for neighbor excess `0.3980`, a `+0.0649` gain. The gain comes from a larger reduction in distant-patch similarity than adjacent-patch similarity.
- Centered spatial contrast: After removing each image's patch-token mean, baseline adjacent/nonadjacent cosine was `0.5459/-0.0469`, for neighbor excess `0.5928`. Specialization was `0.5573/-0.0472`, for neighbor excess `0.6045`, a smaller `+0.0117` gain.
