...
- Run the non-model event controller directly for new managed launches with the active `--study-id`. Do not wrap it in a generic notify-wake process watch because the generic watch and research notifier use different goal-wait source identities. Direct Unix-socket delivery is the default, and the CLI discovers the running daemon socket. It watches durable state with inotify, supervisor exits with pidfds, and progress deadlines with local timers. Study-scoped delivery prevents a stale notification from another study from disarming the active wake path. Wake once after the first train-validation-checkpoint cycle, on exceptional safety events, and on terminal state. Never wake for routine progress writes or notification retry writes.
