...
- Before managed child spawn, capture the live originating thread's effective named permission profile and approval policy from app-server and persist them in the run's immutable `wake-context.json`. Never hardcode a permission profile or replace an existing run context.
