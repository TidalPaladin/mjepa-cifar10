...
- The default ViT-S/4 layout is one CLS token, seven register tokens, four independent CLS predictor partitions, separate global/visual normalization paths, and separate QKV projections in the first four blocks. Use `vit-small-shared-path-legacy.yaml` for checkpoints trained with shared global/visual paths and `vit-small-four-cls-legacy.yaml` only for the former four-CLS baseline.
...
- After verifying supervisor identities, durable startup state, and the armed controller for a dispatched round, enter an owned notify wait when the goal is active, no immediate work remains, and the goal API permits blocking. Do the same after a nonterminal lifecycle event when no immediate mutation remains. Reactivate only a blocked goal whose exact identity and `updatedAt` match an acknowledged owned lease. Treat unmatched, changed, or uncertain blocked goals as manually blocked.
